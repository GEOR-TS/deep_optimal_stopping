...
executive_price=2
#print(tau)
I=2*torch.ones(N).cuda()
#print(I)

s_0=2*torch.ones(dim).cuda()
delta=0.1*torch.ones(dim).cuda()
sigma=0.2*torch.ones(dim).cuda()
corr=0*torch.ones(dim,dim).cuda()
...
def std_Brown_Motion_path(expiry_time):     #生成标准布朗运动样本路径(gpu版)
    Z_walk = torch.randn(expiry_time).cuda()
    B_1=torch.zeros(expiry_time+1).cuda()
    for i in range(expiry_time):
        B_1[i+1]=B_1[i]+Z_walk[i]
    return B_1


def multi_sample_Brown_Motion(num_sample,expiry_time,initial_value):   #生成多个布朗运动路径(gpu版)
    B = initial_value*torch.ones(num_sample, expiry_time + 1).cuda()
...
def high_dim_asset_price_path(s_0,delta,sigma,corr,K,dim,N):  #生成多个高维资产价格样本路径(gpu版)
    S=torch.ones(K,dim,N+1).cuda()
    for k in range(K):
        for d in range(dim):
            B = multi_sample_Brown_Motion(K,N,0)
            S[k,d,:]=s_0[d]*S[k,d,:]
            for i in range(N):
                S[k, d,i+1] = S[k,d,0] * torch.exp((r - delta[d] - ((sigma[d] ** 2) / 2)) * (Time(i+1,N)) + sigma[d] * B[k, i + 1]).cuda()
#    print(S)
    return S
...
def payoff(r,execute_price,n,N,s_k):  #n时刻的回报函数(注意s_k是某一特定样本)(gpu版)
    g=torch.exp(torch.tensor(-r*Time(n,N)))*torch.relu(torch.max(s_k[:,n]-execute_price)).cuda()
    return g
...
def continuation_brown_motion_path(B,k,K,n,N,J):   #某一样本点的多个布朗运动延续样本路径(gpu版)
...
def contiuation_high_dim_asset_path(S,delta,sigma,corr,dim,k,K,n,N,J):  #某一样本点的价格过程的延续样本路径(gpu版)
...
class stopping_determine_model(nn.Module):     #构建两层神经网络类(gpu版)
    def __init__(self,n,dim,q_1):
        super(stopping_determine_model, self).__init__()
        self.n=n
        self.dim=dim
        self.q_1=q_1
        self.net=nn.Sequential(nn.Linear(self.dim,self.q_1).cuda(),nn.ReLU().cuda(),nn.Linear(self.q_1,1).cuda(),nn.LogSigmoid().cuda()).cuda()
...
def n_k_approxi_expect_reward(S,k,K,n,N,all_determine_func):   #计算第n时刻k样本的回报估计(gpu版)
    if n == N:
...
def n_average_approxi_expect_reward(S,K,n,N,all_determine_func):   #第n时刻的平均回报估计(gpu版)
    sum_reward=0
...
def n_train_func(all_determine_func,lr,S,K,dim,n,N,q_1,batch_size=5,num_epochs=5):  #n时刻决策函数的训练（N除外）(gpu版)
    data_iter=data.DataLoader(data.TensorDataset(S),batch_size,shuffle=True)
    target_train_obj=stopping_determine_model(n,dim,q_1).cuda()
...
def optimize_all_determine_func(S,K,dim,N,q_1,all_determine_func,lr):   #倒向迭代（循环）训练所有时刻的决策函数(gpu版)
    #先计算N时刻的决策函数
    ...
    #倒向迭代计算所有决策函数
    for n in range(N):
        n_train_func(all_determine_func,lr,S,K,dim,N-n-1,N,q_1)
...
def n_k_conti_path_sample_set_dim_expansion(J_n_k,J,dim,n,N):    #向前增加0使得新矩阵跟我们N时刻的矩阵在n方向分量维数相同，使得格式统一便于使用前面已设函数(gpu)版
    New_matrix=torch.zeros(J,dim,N+1).cuda()
    for m in range(N-n+1):
        New_matrix[:,:,n+m]=J_n_k[:,:,m]
    return New_matrix

def n_k_continuation_value(J_n_k,all_determine_func, n, N, J):  # 计算延续路径价值(n小于N)，J_n_k代表新生成第k个样本n时刻的J个延续样本路径的样本集(注意这里的J_n_k是维数扩张后的样本集)(gpu版)
    Val = 0
...
def UpperBound_and_variance(New_Sample,all_determine_func,delta,sigma,corr,dim,K_U,J,N):    #用新样本集计算上界和渐近方差，为统一记号方便计算和理解，我们令n=0的delta_M=0，我们先用集合将所有delta_M算出来后再进行求和（以list返回）
    ...
    #计算上界
    for k in range(K_U):
        delta_M_k = torch.zeros(N + 1).cuda()
        M_k=torch.zeros(N+1).cuda()
        payoff_k=torch.zeros(N+1).cuda()
        for n in range(N+1):
            # 计算n_k处payoff
            ...
            M_k[n]=M_k_n
        #计算sum
        max_difference=torch.max(payoff_k-M_k).cuda()                #计算n从0到N中的最大差
...
def Point_est_and_Confidence_interval(alpha,LowBd_with_var,UppBd_with_var,K_L,K_U):        #计算点估计和置信区间(以list返回)
    ...
    #计算置信区间
    Interval_L=LowBd_with_var[0]-z_alpha_half*(LowBd_with_var[1]/torch.sqrt(torch.tensor(K_L).cuda()).cuda())
    Interval_U=UppBd_with_var[0]+z_alpha_half*(UppBd_with_var[1]/torch.sqrt(torch.tensor(K_U).cuda()).cuda())
...
#        print('\n')
        for k in range(10):
            l = loss(F_N.forward(S_k_inlist[k, :, N]), torch.tensor(1).cuda())
            optimizer_N.zero_grad()
            l.backward()
            optimizer_N.step()
#            l = loss(F_N.forward(S_k_inlist[k,:, N]), torch.tensor(1))
